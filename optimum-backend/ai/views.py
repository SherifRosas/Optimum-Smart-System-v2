...
from ai_services import OrderAnalysisAI, AIInsights, ChatAssistantAI, SmartOrderSuggestions, PredictiveAlerts
...
def get_ai_dashboard_data(request):
    ...
    try:
        ...
        print(f"AI Dashboard Error: {error_trace}")
        return Response({
            'success': False,
            'error': str(e),
...
def chat(request):
    """
    AI Chat endpoint - conversational chat with context
    """
    try:
        ...
        chat_ai = ChatAssistantAI()
        
        # Get system context (orders data for contextual answers)
        orders_data = []
        if context.get('include_orders', False):
            orders = Order.objects.all()[:10]  # Recent orders
            orders_data = [OrderSerializer(order).data for order in orders]
        ...
        # Get AI response
        ai_result = chat_ai.chat(user_message, conversation_history, system_context)
        ...
        chat_message = ChatMessage.objects.create(
            ...
            message_type=ai_result.get('message_type', 'general'),
            context_data=system_context
        )
        
        # Update session
        session.updated_at = chat_message.created_at
        session.save()
...
def get_smart_suggestions(request):
    ...
    try:
        ...
        suggestions_service = SmartOrderSuggestions()
        suggestions = suggestions_service.get_suggestions(orders_data)
...
@api_view(['GET'])
@permission_classes([permissions.IsAuthenticated])
def get_predictive_alerts(request):
    """
    Get predictive alerts based on order data
    """
    try:
        orders = Order.objects.all()
        orders_data = [OrderSerializer(order).data for order in orders]
        
        alerts_service = PredictiveAlerts()
        alerts = alerts_service.generate_alerts(orders_data)
