...
class Order(models.Model):
    ...
    is_ai_generated = models.BooleanField(default=False, help_text="Whether this order was automatically generated by AI")
    # Confirmation and feedback
    is_delivered_confirmed = models.BooleanField(default=False, help_text="Whether customer confirmed delivery")
    customer_feedback = models.TextField(blank=True, null=True, help_text="Feedback from customer")
    ...
    risk_score = models.FloatField(default=0.0, help_text="AI-calculated risk score (0.0 to 1.0)")
...
class SupplierOffer(models.Model):
    ...
    order = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name='offers',
        help_text="Order this offer is for"
    )
    supplier = models.ForeignKey(
        Supplier,
        on_delete=models.CASCADE,
        related_name='offers',
        help_text="Supplier making the offer"
    )
    unit_price = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        help_text="Price per unit offered"
    )
    total_amount = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        help_text="Total amount for this offer"
    )
    status = models.CharField(
        max_length=20,
        choices=STATUS_CHOICES,
    ...
    class Meta:
        ordering = ['unit_price']  # Best prices first
        unique_together = ['order', 'supplier']

    def __str__(self):
        return f"Offer for Order #{self.order.id} from {self.supplier.name} at ${self.total_amount}"

    def save(self, *args, **kwargs):
        self.total_amount = self.order.quantity * self.unit_price
        super().save(*args, **kwargs)
