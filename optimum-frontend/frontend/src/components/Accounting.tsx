...
const Accounting: React.FC<AccountingProps> = ({ orders }) => {
    ...
    const getSupplierCost = (order: Order): number => {
        return getOrderValue(order) * 0.8;
    };

    const getProfit = (order: Order): number => {
    ...
    const ordersByStatus = useMemo<Record<string, number>>(() => {
    ...
    const topProducts = useMemo<TopProductItem[]>(() => {
